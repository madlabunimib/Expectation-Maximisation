...
list_missing_soft <- function(initial_data) {
  lista_missing = c()
  missing = FALSE
  for (j in rownames(initial_data)) {
    ...
    if (missing == TRUE) {
      lista_missing <- c(lista_missing, j)
    }
    missing = FALSE
...
#return the list of the record with at least a missing value. It is important for the expectation step
prob_missing_soft <- function(Data,initial_data) {
  matrix.missing.prob = matrix(0,nrow = nrow(Data), ncol=ncol(Data))
  dimnames(matrix.missing.prob) <- list(rownames(Data),colnames(Data))
  
  for (i in 1:ncol(initial_data)) {
    for (j in 1:nrow(initial_data)) {
      if (is.na(initial_data[j,i])) {
        matrix.missing.prob[j,i] = 1
      }else {
        matrix.missing.prob[j,i] = "observed"
      }
    }
    
  }
  return(matrix.missing.prob)
}


get_not_observed_node_soft <- function(node_to_compute, lista_missing_variable, cpt) {
...
expectation_step_soft <- function(missing_data,table_posterior_prob,matrix.missing.prob,Data,bn,cpt) {
  ...
  for (i in missing_data) {
    ...
    for (j in colnames(matrix.missing.prob)){
      if (matrix.missing.prob[i,j] != "observed"){
        node_to_compute = j
    ...
    for (node_to_compute in lista_missing_variable){
      ...
      if (length(lista_missing_variable) > 1) {
        matrix_node_not_observed = get_not_observed_node_soft(node_to_compute, lista_missing_variable,cpt)
        all_evidence = FALSE
      }
      ...
      if (all_evidence == TRUE) {
        ...
        for (factor in nodes(bn)){
          ...
          prior_value = FALSE
          if (matrix.missing.prob[i,factor] == "observed"){
            for (k in colnames(prior_observation)){
              prior_observation[1,k] = as.numeric(Data[i,factor])+1
              prior_value = TRUE
            }
          ...
          if (identical(lista_genitori_factor,character(0))){
            ...
            for (x in genitore_factor){
              ...
              if (matrix.missing.prob[i,x] == "observed"){
                
                for (k in colnames(genitore_observation)){
                  genitore_observation[1,k] = as.numeric(Data[i,x])+1
                }
        ...
        for (combination in 1:nrow(matrix_node_not_observed)){
          ...
          fit_data = Data
          fit_Mprob = matrix.missing.prob
          for (column in colnames(matrix_node_not_observed)){
            fit_data[i,column] = as.integer(matrix_node_not_observed[combination, column]) - 1
            fit_Mprob[i,column] = "observed"
          }
          ...
          for (factor in nodes(bn)){
            ...
            colnames(prior_observation) = row.names(cpt[[factor]])
            prior_value = FALSE
            if (fit_Mprob[i,factor] == "observed"){
              for (k in colnames(prior_observation)){
                prior_observation[1,k] = as.numeric(fit_data[i,factor])+1
                prior_value = TRUE
              }
            ...
            if (identical(lista_genitori_factor,character(0))){
              ...
              for (x in genitore_factor){
                ...
                if (fit_Mprob[i,x] == "observed"){
                  
                  for (k in colnames(genitore_observation)){
                    genitore_observation[1,k] = as.numeric(fit_data[i,x])+1
                  }
...
update_data_soft <- function(missing_data,table_posterior_prob,Data) {
...
stopping_criteria_soft <- function(alpha, cpt_last, cpt,bn) {
  delta = 0
...
maximisation_step_soft <- function(node,bn,table_posterior_prob,Data,cpt,matrix.missing.prob) {
    ...
    if (identical(lista_genitori,character(0))) {
      ...
      for (i in rownames(Data)) {
        for (np in colnames(numerator_prior)){
          if (Data[i,j] == np && matrix.missing.prob[i,j] == "observed") {
            numerator_prior[1,np] = numerator_prior[1,np] + 1
          } else if (matrix.missing.prob[i,j] != "observed"){
      ...
      for (j in rownames(matrix_to_compute)) { 
        ...
        for (i in rownames(Data)) {
          ...
          for (parent in genitore){
            if (Data[i,parent] == matrix_to_compute[j,parent] && matrix.missing.prob[i,parent] == "observed"){
              out = TRUE
            } else if (!matrix.missing.prob[i,parent] == "observed"){
              missing = TRUE
          ...
          if (out == TRUE && missing == FALSE ) {
            ...
            for(vn in 1:values_node){
              if (Data[i,node] == vn-1 && matrix.missing.prob[i,node] == "observed") {
            ...
            if (matrix.missing.prob[i,node] != "observed") {
...
Create_matrix_posterior_prob_soft <- function(missing_data, matrix.missing.prob,bn) {
...
initialize_matrix_posterior_prob_soft <- function(missing_data, matrix.missing.prob,table_posterior_prob,bn){
  for (i in missing_data){
    for (j in colnames(matrix.missing.prob)){
      if (matrix.missing.prob[i,j] == "observed"){
...
check_input_soft <- function(initial_data,cpt,ALPHA,NI){
...
standardize_data_soft <- function(initial_data,cpt){
  if(length(cpt) == nrow(initial_data) && names(cpt[1]) %in% rownames(initial_data)){
    initial_data <- data.frame(t(initial_data), stringsAsFactors = FALSE)
  }else if(length(cpt) == ncol(initial_data) && names(cpt[1]) %in% colnames(initial_data)){
  }else{
...
em_soft <- function(initial_data, structure, cpt, ALPHA=0.05, NUMBER_ITERACTION=15){
  results=list()
  NUMBER_ITERACTION = as.integer(NUMBER_ITERACTION)
  check_input_soft(initial_data,cpt,ALPHA, NUMBER_ITERACTION)
  Data = standardize_data_soft(initial_data,cpt)
  
  
  cpt_last = cpt
  bn = custom.fit(structure,cpt)
  #variable that contains all the missing variables (important to expectation step)
  missing_data = list_missing_soft(initial_data)
  matrix.missing.prob = prob_missing_soft(Data,initial_data)
  #Inizio programma
  table_posterior_prob = Create_matrix_posterior_prob_soft(missing_data, matrix.missing.prob,bn)
  table_posterior_prob = initialize_matrix_posterior_prob_soft(missing_data, matrix.missing.prob,table_posterior_prob,bn)
  
  
  for (i in 1:NUMBER_ITERACTION) {
    table_posterior_prob = expectation_step_soft(missing_data,table_posterior_prob,matrix.missing.prob,Data,bn,cpt)
    Data = update_data_soft(missing_data,table_posterior_prob,Data)
    cpt = maximisation_step_soft(node,bn,table_posterior_prob,Data,cpt,matrix.missing.prob)
    stop_criteria = stopping_criteria_soft(ALPHA, cpt_last, cpt,bn)
