...
expectation_step_hard <- function(missing_data,table_posterior_prob,matrix.missing.prob,Data,bn,cpt) {
...
maximisation_step_hard <- function(node,bn,table_posterior_prob,Data,cpt) {
  ...
  if (identical(parents,character(0))) {
    ...
    for(data.index in 1:nrow(Data)){
      for(row.index in 1:nrow(compute_probabilities)){
        is_correct_row = TRUE
        for(parent.variable in parents){
          if(compute_probabilities[row.index,parent.variable] != Data[data.index,parent.variable]){
        ...
        if(is_correct_row){
          value = as.numeric(Data[data.index,node])

          if (Data[data.index,node] == 0.5){
            ok = 1
          }
...
Create_matrix_posterior_prob_hard <- function(Data, missing_data, matrix.missing.prob,bn) {
...
initialize_matrix_posterior_prob_hard <- function(Data, missing_data, matrix.missing.prob,table_posterior_prob,bn){
  for (i in missing_data){
    for (j in colnames(matrix.missing.prob)){
      if (matrix.missing.prob[i,j] == "observed"){
...
em_hard <- function(initial_data, structure, cpt, ALPHA=0.05, NUMBER_ITERACTION=20){
  ...
  matrix.missing.prob = prob_missing_hard(Data,initial_data)

  #Inizio programma
  table_posterior_prob = Create_matrix_posterior_prob_hard(Data,missing_data, matrix.missing.prob,bn)
  table_posterior_prob = initialize_matrix_posterior_prob_hard(Data,missing_data, matrix.missing.prob,table_posterior_prob,bn)
  

  for (i in 1:NUMBER_ITERACTION) {
    
    
    table_posterior_prob = expectation_step_hard(missing_data,table_posterior_prob,matrix.missing.prob,Data,bn,cpt)
